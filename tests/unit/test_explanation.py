...
The per-topic "revision suggestion" was removed (editing is the proposing lecturer's job); these
tests now cover the breakdown, shared concepts, reasons, level and action only.
"""
...
def test_report_no_longer_carries_revision_suggestion():
    rep = build_explanation(_HOTEL, _PHARMACY, _scores(0.4, 0.5, 0.98, 0.20, 0.55, "Moderate"))
    assert "revision_suggestion" not in rep
...
# ── both-domain overlap ─────────────────────────────────────────────────────────────
def test_both_domain_and_structure():
...
def test_same_domain_different_stack_shares_domain():
...
# ── low similarity ──────────────────────────────────────────────────────────────────
def test_low_similarity_action_accept():
